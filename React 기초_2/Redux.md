...
**변경 불가능**이란 말은 **상태 객체 내부가 바뀌지 않는다는 뜻**입니다. 상태에 어떠한 변화가 필요하게 될 때는 객체 전체를 바꿔치기하는 방식을 사용하고, Action(액션)이 그렇게 바꿀 대상을 지정합니다. 액션은 애플리케이션 상태 중에서 어떤 부분을 바꿀지 지정하고 그런 변경에 필요한 데이터를 제공합니다.
...
Redux를 사용하는 것이 reducer를 작성하는 일이라고 말해도 과언이 아닐 만큼 reducer는 Redux의 아주 핵심적인 부분입니다.
